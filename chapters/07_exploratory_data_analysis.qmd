...
# Intro to Exploratory Data Analysis with Python
...
- [pandas](https://pandas.pydata.org/): Python's data frame library
- [Jupyter](https://jupyter.org/) or [Quarto](https://quarto.org/)
...
```{python}
state_demographics = pd.read_csv("../_data/corgis/state_demographics.csv")
```
...
- One thing to call out is setting the minimum and maximum values of the color palette. You won't always need to do that, or even if you do, you won't always need to set all three. But it is a good idea to be aware of it, as you want to ensure the correct part of the color palette covers the correct part of the data.
- `cbar_kws`: https://matplotlib.org/stable/api/_as_gen/matplotlib.figure.Figure.colorbar.html (and you know that because the docs say to go there and figure out the keyword args! This is a _very_ common thing in docs. They'll be like "oh, just go here and you can see the arguments that work". It's because libraries will often pass arguments through directly to other functions.)
- What about the type of linkage and the distance? See here: https://docs.scipy.org/doc/scipy/reference/generated/scipy.cluster.hierarchy.linkage.html#scipy.cluster.hierarchy.linkage
...
```{python}
cancer = pd.read_csv("../_data/corgis/cancer.csv")
cancer.head()
```
...
In the state demographics data, we saw that strong dependency between some of the variables and state population. We would also expect there to be a pretty strong dependency between the total number of cancer deaths and the total state population.
...
Let's get an overall idea of different states and cancer. Since we would expect states with more people to naturally have more deaths due to cancer, we will use the rates per 100k people again.
...
```{python}
disease = pd.read_csv("../_data/corgis/health.csv")
disease.head()
```
...
```{python}
coffee = pd.read_csv("../_data/corgis/coffee_small.csv")
coffee
```
